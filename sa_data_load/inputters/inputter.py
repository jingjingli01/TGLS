...
from sa_data_load.inputters.dataset_base import UNK_WORD, PAD_WORD, BOS_WORD, EOS_WORD
from sa_data_load.inputters.text_dataset import TextDataset
from sa_data_load.inputters.image_dataset import ImageDataset
from sa_data_load.inputters.audio_dataset import AudioDataset
from sa_data_load.utils.logging import logger
...
def _load_fields_2(dataset, data_type, data_dir, checkpoint):
    if checkpoint is not None:
        logger.info('Loading vocab from checkpoint at %s.' % opt.train_from)
        fields = load_fields_from_vocab(
            checkpoint['vocab'], data_type)
        ...
        fields = load_fields_from_vocab(
            torch.load(data_dir + '.vocab.pt'), data_type)
    fields = dict([(k, f) for (k, f) in fields.items()
                   if k in dataset.examples[0].__dict__])
